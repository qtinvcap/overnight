...
def get_price_acceleration(df):
    ...
    """
    # Define the different time windows.
    df_last5_min = df[
    ...
    def compute_acceleration(group):
        ...
        first_diff = group["close"].pct_change()
        acceleration = first_diff.diff()
        return acceleration.mean()

    # Compute acceleration for each window, ensuring output is 1-dimensional.
    if df_last5_min.empty:
        tickers = df["ticker"].unique()
        acceleration_feature_5min = pd.Series(np.nan, index=tickers, name="intraday_last5_price_acceleration")
    else:
        acceleration_feature_5min = df_last5_min.groupby("ticker").apply(compute_acceleration)
        acceleration_feature_5min.name = "intraday_last5_price_acceleration"
...
def assemble_all_intraday_features(df):
    ...
    (
        last5_price_acceleration,
        last_30min_price_acceleration,
        last_hour_price_acceleration,
        second_half_day_price_acceleration,
    ...
    data = {
        ...
        # Price acceleration
        "intraday_last5_price_acceleration": last5_price_acceleration,
